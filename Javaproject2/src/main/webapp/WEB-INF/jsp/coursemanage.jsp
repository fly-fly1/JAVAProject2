...
<c:forEach begin="0" end="${list.size()-1}" var="i">
	<tr>
		...
		<td>${list[i].subject.subject_name}</td>
		<td>${list[i].course_desc}</td>
		<td><a href="course_update?id=${list[i].id}">□</a></td>
		<td><a href="course_delete?id=${list[i].id}">X</a></td>
	</tr>
</c:forEach>
</c:if>
</table>
<!-- pages:总页数；count:总记录数 -->
	...
	<c:forEach var="i" begin="1" end="${p}">
		<c:if test="${page==i}">
			<a href="course_manage?page=${i}"><i style="color: red">第${i}页</i></a>
		</c:if>
		<c:if test="${page!=i}">
			<a href="course_manage?page=${i}">第${i}页</a>
		</c:if>
	</c:forEach>
<script type="text/javascript">
	function add() {
		location.href="course_add";
	}
	function deleteAll() {
...
//			contentType: "application/json; charset=utf-8",
			...
			success: function(data){
				if(data=='success'){
					alert("温馨提示：删除成功");
					document.location.href="course_manage";
				}else{
					alert("温馨提示：删除失败");
				}
         	 }
