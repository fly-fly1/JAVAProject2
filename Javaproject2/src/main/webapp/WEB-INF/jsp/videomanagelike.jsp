...
<c:forEach begin="0" end="${list.size()-1}" var="i">
	...
	<c:forEach var="i" begin="1" end="${p}">
		<c:if test="${page==i}">
			<a href="video_manageLike?page=${i}&title=${title}&speaker_id=${speaker_id}&course_id=${course_id}"><i style="color: red">第${i}页</i></a>
		</c:if>
		<c:if test="${page!=i}">
			<a href="video_manageLike?page=${i}&title=${title}&speaker_id=${speaker_id}&course_id=${course_id}">第${i}页</a>
		</c:if>
	</c:forEach>
